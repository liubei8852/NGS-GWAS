...
open VCF,">$outpath/$name.miss$miss.maf$maf.het$het\.vcf\.txt" || die "can not open $!.\n";
open VCF1,">$outpath/$name.miss$miss.maf$maf.het$het\.vcf\.all\.txt" || die "can not open $!.\n";
open GENO,">$outpath/$name.miss$miss.maf$maf.het$het\.geno\.txt" || die "can not open $!.\n";
open GENO1,">$outpath/$name.miss$miss.maf$maf.het$het\.geno\.gapit.txt" || die "can not open $!.\n";
open TXT,">$outpath/$name.miss$miss.maf$maf.het$het.xls" || die "can not open $!.\n";
open GENO2,">$outpath/$name.miss$miss.maf$maf.het$het\.geno\.maf\.txt" || die "can not open $!.\n";
##save sample name##
while (my $line = <IN>){
	chomp $line;
	if ($line=~/^##/){
...
my @seq1;my @seq2;my @sep3;my $i=0;my $j=0;my $mk=0;my $mk0=0;my $mk1=0;my $sum1=0;my $total=0;my $sum=0;my $het1=0;
my $i1;my $i2;my $i3;my $sum2;my $miss01=0;my $miss02=0;my $miss03=0;my $miss04=0;my $miss05=0;my $double=0;my $sumgqa=0;
while (my $line = <IN>){
	chomp $line;
...
##filter individual low quality &&individual depth##	
      ...
      if ($sep1[$i] =~ /^\d/){
        ...
        if  (($sep5[$dp] =~/\d/) &&($sep5[$gq] =~ /\d/)&& ($sep5[$dp] >= $dpmin) && ($sep5[$dp] <= $dpmax) && ($sep5[$gq] >= $gqs)){
          ...
          elsif (((split(/\:/,$sep1[$i]))[0])eq "0/1"){$seq3[$i]=$seq2[$i]="$sep2[3]$sep2[4]";$mk0++;$mk1++;$het1++;}
...
##filter  miss && maf ##		
    ...
    if ($j==$pop){next;}
      ...
      #print "$pop\t$j\t$mk\t$diff\n";
      if ($het1>1){$het1=$het1*2/($mk);}
			if ($mk0==0){$mk=1;}
			$i2=$mk0/($mk);
			$i3=$mk1/($mk);
			if ($i2>$i3){$i2=$i3;}
			if(($i2 >=$maf) && ($i3 >= $maf)&&($het1 >=$het)){
				$sum++;
